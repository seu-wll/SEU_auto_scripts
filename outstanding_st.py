...
#开启服务为了后续关闭服务，不然进程一直杀不掉
driver_service = Service( './webdriver/chromedriver.exe')  #括号内填写 驱动路径
...
with open('login_information.csv') as f:    #read students information from csv
    ...
    for row in reader:
        if len(row)>1:            
                browser = webdriver.Chrome('./webdriver/chromedriver.exe',chrome_options=chrome_options)              
                ...
                a=0
                while not len(browser.find_elements_by_css_selector('div.bh-btn.bh-btn-primary')):
                    time.sleep(1)
                    a=a+1
                    if a>60 :
                        browser.quit()
